...
class CodexTaskMapperTest {
    ...
    fun `reads nested last and total usage`() {
        ...
        assertEquals(AgentUsage(inputTokens = 30, cachedInputTokens = 0, outputTokens = 10, reasoningTokens = 0, totalTokens = 40, cacheWriteInputTokens = 0), changed.task)
        assertEquals(140, changed.session?.totalTokens)
    }

    @Test
    fun `preserves cache write input tokens independently from cache hits`() {
        ...
        val changed = assertIs<TaskEvent.UsageChanged>(events.first())
        assertEquals(4, changed.task.cacheWriteInputTokens)
        assertEquals(9, changed.session?.cacheWriteInputTokens)
        assertEquals(0, changed.task.cachedInputTokens)
    }
