...
## provider 고유 기능의 출입 경계

공통 계약이 다루지 않는 provider 고유 기능은 방향에 따라 경로가 다르다. 두 경로는 대칭이 아니다.
...
이전 Port의 `metadata`를 제거한 이유가 이것이다. 의미가 정해지지 않은 값을 통과시켜 어떤 adapter도 이행을 약속하지 않았고 소비자도 적용 여부를 관찰할 수 없었다. 상관관계 식별이라는 목적 자체는 [선택 계약](capability-candidates.md)의 검토 항목으로 남아 있으며, 채택한다면 목적과 관찰 가능한 보장을 갖춘 타입으로 정의한다. 이름을 되살려 통과 경로로 쓰지 않는다.
