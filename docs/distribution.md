...
소스의 factory는 새 Port로 전환했고 기본 publication version은 `0.2.0`이다. artifact는 이번 작업에서 발행하지 않았으며 기존 `0.1.0` 다운로드가 새 AgentTask API를 제공한다고 주장하지 않는다.
...
./gradlew.bat bundleForMavenCentral -PpublicationVersion=0.2.0
```
