...
현재 행렬은 Codex 두 구성, Gemini CLI 두 구성, Koog 한 구성의 요구 58개를 다룬다. preflight/direct 실행 116개와 profile 지원표 5개, 총 121개다. 영속성·승인·질문·진단·workspace·실행 제약·구조화 산출물·reasoning option 선택의 지원과 사전 거절을 구별한다. Reasoning option 사례는 세 comprehensive profile에서 목록 밖 값을 모델 실행 전에 거절하며, Codex adapter의 catalog 미관측 preflight와 실제 admission 갱신은 SDK contract suite에서 별도로 검증한다.
...
G09 workspace/실행 7개와 reasoning option 사례를 포함한 요구 판정 121개를 합치면 현재 행렬은 128개다. 기존 122개 checkpoint는 reasoning option 추가 전 실행이며, 새 선택 계약의 증거는 G01과 adapter/host 검증에 구분해 기록한다. 복합 명령의 “아무 효과도 없음”을 개별 상한의 증거로 재사용하지 않았다.
...
갱신된 요구 판정 증거의 세 native 구성 × 검증 책임 두 종류 × preflight/direct = 12개 요구 검사가 이 조건을 검증한다. 다섯 profile의 지원 표 검사도 미지원을 확인한다. 121개 요구 판정 실행에 포함된 검사이며 신규 12개로 더하지 않는다. 선택 기능을 새로 구현하거나, 미지원 선택 보장을 기본 계약으로 강제하지 않았다. 향후 schema 실행 경로를 추가하면 VALID/INVALID/NOT_VALIDATED와 부분 산출물의 실제 의미를 별도 실증해야 한다.
