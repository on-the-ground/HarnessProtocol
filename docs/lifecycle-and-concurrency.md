...
`Unresolved`를 전달할 때는 위 네 조건에 대응하는 `UnresolvedReason`을 함께 남긴다. OBSERVATION_LOST는 관찰 경로만 끊긴 경우, CLEANUP_BOUND_EXCEEDED는 공개한 정리 상한 안에서 종결을 확인하지 못한 경우, CANCELLATION_UNCONFIRMED는 취소 요청은 수락됐으나 실제 종료를 확인하지 못한 경우, PARTIAL_EVIDENCE는 일부 종료 사실만 아는 경우다. 후속 복구 행동이 사유마다 다르므로 하나로 뭉뚱그리지 않는다.
...
Adapter는 호출자가 사용 전에 알 수 있도록 작업 하나의 유예 시간과 close/release 전체의 상한, 여러 자원의 유예가 순차 합산되는지를 `AgentHarness.cleanupBudget`의 `perTask`·`total`·`aggregatesAcrossResources`로 제공한다. 이는 필수 프로퍼티이며 문서화로 대신하지 않는다. 공통 계약에 특정 process나 2초 유예를 고정하지 않는다. 유예 만료를 취소 성공으로 바꾸지 않는다. 이미 종결된 작업을 close가 덮어쓰지 않는다.
