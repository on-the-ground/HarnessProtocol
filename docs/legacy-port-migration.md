...
루트 JVM 105개(native 25개 포함), 독립 Koog 실험 18개, host 20개가 모두 통과했다. 소스·빌드 스크립트와 바이너리·소스 JAR에서 구 Port 참조 0건을 확인했고, 문서 링크와 diff 공백 검사도 통과했다. 개별 실행 수와 결과는 위 기계 판독 기록을 따른다. legacy 제거는 모든 선택 계약과 경쟁 조건의 적합성 인증을 뜻하지 않는다.
