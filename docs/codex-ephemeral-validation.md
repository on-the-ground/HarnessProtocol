...
- Maven Central의 `io.github.joohyung-park:harness-protocol`과 `harness-codex` 최신 공개 버전은 `0.2.0`이다. 공개 core sources와 배포 기준 `main`을 모두 확인했다.
- 검토한 최신 main 기준은 release bump `37015e50600b32552f1f28dc49d40cb6ddf0edaf`다. 공개 `0.2.0`과 이 revision 모두 provider retention 요구, user-history visibility 요구, 생성 결과 disposition을 제공하지 않았다.
